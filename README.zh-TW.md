...
- **手動選擇模式**：透過勾選框手動選擇哪些提示詞要成為資料夾標題，不需要特殊命名規則。
- **群組連動控制**：
  - **摺疊/展開**：點擊標題可收合內容。
  - **啟用/停用**：當你停用（Disable）群組標題時，該群組內的所有提示詞都會被自動過濾，**不會發送給 AI**。這讓你能在不同場景間快速切換整組設定。
- **跨 Preset 複製配置**：可從其他 Preset 複製摺疊配置，透過 UUID 匹配讓你在 Preset 更新時也能輕鬆遷移設定。
- **折設定**：設定面板中的「折設定」勾選框，可將 OpenAI API 參數滑桿（上下文長度、溫度等）折疊成一個可展開的區塊，讓介面更整潔。狀態存於 localStorage，不受 Preset 切換影響。
...
3.  **工具列按鈕**：
      ...
      - `⚙️`：開啟設定面板，切換模式、自訂分隔符號、開啟 Debug 模式或折疊 API 設定區塊。

4.  **跨 Preset 複製配置**：
      - 開啟設定面板（⚙️ 按鈕）
      - 在「從其他 Preset 複製配置」區塊選擇來源 Preset
      - 點擊「複製」按鈕
      - 系統會自動透過 UUID 匹配，將摺疊配置套用到當前 Preset

5.  **折設定**：
      - 開啟設定面板（⚙️ 按鈕）
      - 勾選「折設定」勾選框
      - OpenAI API 參數滑桿（上下文長度、溫度、各種 Penalty 等）會折疊成一個可展開的「詳細設定」區塊
      - 此設定存於 localStorage，切換 Preset 後仍然保持
